...
def _render_incident_assistant_for_test() -> None:
...
def test_incident_assistant_sends_only_selected_incident_context() -> None:
    response = {
        "incident_id": "inc-ui-test",
        "answer": "Stripe is the strongest supported provider-level cause.",
        "answerable": True,
        "evidence": [
            {
                "fact_id": "diagnosis_evidence_1",
                "label": "Diagnosis evidence 1",
                "value": "provider = Stripe; live approval 54.0%.",
            }
        ],
        "mode": "mock",
    }
    with patch(
        "frontend.incident_assistant_ui.ask_incident_assistant",
        return_value=response,
    ) as ask:
        app = AppTest.from_function(_render_incident_assistant_for_test).run(timeout=20)
        app.chat_input[0].set_value("What is the root cause?").run(timeout=20)
    ...
    assert any("Stripe" in markdown.value for markdown in app.markdown)


def test_assistant_client_uses_incident_scoped_endpoint() -> None:
    response = Mock(ok=True)
    response.json.return_value = {"answer": "Grounded answer."}
    ...
    request.assert_called_once_with(
        "POST",
        "http://api.test/incidents/inc-1/assistant",
        json={"merchant": "Rappi", "question": "What is the impact?"},
        params=None,
        timeout=5,
    )
